      ...
      ${concatStringsSep "\n" (map (
          cmd: let
            opts = filter (s: s != "") [
              ...
              (
                if cmd.group != null
                then "group = ${genLua cmd.group}"
                else ""
              )
              (
                if cmd.callback != ""
                then "callback = ${cmd.callback}"
                else ""
              )
              (
                if cmd.command != null
                then "command = ${genLua cmd.command}"
                else ""
              )
              (
                if cmd.once
          ...
          in "vim.api.nvim_create_autocmd(${genLua cmd.event}, { ${concatStringsSep ", " opts} })"
        )
        cfg.autocmds)}
    '';
  };
