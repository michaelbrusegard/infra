...
let
  ...
  modeToLua = mode:
    if lib.isList mode then "{ ${concatStringsSep ", " (map (m: toLua {} m) mode)} }"
    else toLua {} mode;

  mkOpts = k: 
    let 
      opts = ["silent = ${toLua {} k.silent}"]
  ...
  mkKeymapLua = k: 
    let 
      actionStr = if k.lua then k.action else toLua {} k.action;
    in
      ''vim.keymap.set(${modeToLua k.mode}, ${toLua {} k.key}, ${actionStr}, ${mkOpts k})'';
in {
