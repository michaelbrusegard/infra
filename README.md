...
### NetBird + Pocket ID Bootstrap

NetBird uses an embedded IdP for initial setup. Pocket ID is added as the
permanent external OIDC provider afterward. This requires a one-time env
swap in `nix-secrets`.

1. **Temporarily set embedded IdP env** in
   `nix-secrets/gitops/espresso/apps/netbird/secrets.yaml`:

   ```
   AUTH_AUTHORITY: https://netbird.gullhaugveien.michaelbrusegard.com/oauth2
   AUTH_CLIENT_ID: netbird-dashboard
   AUTH_AUDIENCE: netbird-dashboard
   ```

   Push, reconcile `apps-secrets`, restart both NetBird pods.

2. **Create the local admin** at `https://netbird.gullhaugveien.michaelbrusegard.com`.
   Use the same email as your Pocket ID account.

3. **Add Pocket ID** in NetBird dashboard: Settings > Identity Providers >
   Generic OIDC. Use issuer `https://id.gullhaugveien.michaelbrusegard.com`
   with the Pocket ID client credentials.

4. **Promote Pocket ID user**: Log out, log in via Pocket ID, then log back
   in as local admin and approve/promote the Pocket ID user to Owner.

5. **Switch env to Pocket ID permanently**:

   ```
   AUTH_AUTHORITY: https://id.gullhaugveien.michaelbrusegard.com
   AUTH_CLIENT_ID: <pocket-id-client-id>
   AUTH_AUDIENCE: <pocket-id-client-id>
   ```

   Push, reconcile, restart dashboard. Optionally set
   `localAuthDisabled: true` in the server config section.

6. **Create macchiato setup key** in NetBird, put it in
   `nix-secrets/hosts/macchiato/secrets.yaml`, push, rebuild macchiato.

7. **Add routes** for `10.0.186.0/24`, `10.0.187.0/24`, `10.0.188.0/24`.
