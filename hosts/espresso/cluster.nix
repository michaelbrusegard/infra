    ...
    flux-secrets-auth =
      ...
      (config.networking.hostName == "espresso-0")
      {
        ...
        script = ''
          set -euo pipefail
          export KUBECONFIG=/etc/rancher/k3s/k3s.yaml
          test -f ${config.secrets.k3s.flux.secretsDeployKeyFile}
          until ${lib.getExe pkgs.kubectl} get ns flux-system >/dev/null 2>&1; do
            sleep 2
          done
          ${lib.getExe pkgs.kubectl} -n flux-system create secret generic secrets-auth \
            --from-file=identity=${config.secrets.k3s.flux.secretsDeployKeyFile} \
